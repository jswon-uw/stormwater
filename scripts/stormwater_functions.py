...
def calc_snagg(df, aggtype=max, t=0.9):
    df = df.copy()
...
def calc_moagg(df, aggtype=max, t=0.9):
...
def process_stats_df(sdf, pdf, stats, period, min_yrs):
    sdf = sdf[[ x for x in pdf.columns if 'hr' in x]]
    pdf = pdf[[ x for x in pdf.columns if 'hr' in x]]

    df = pd.DataFrame()
    hrs = [1, 2, 3, 6, 12, 24, 48, 72, 120, 240, 360]
        
    # Combine total sum
    ddf = pd.DataFrame(sdf.mean() / hrs).T
    ...
    for col in pdf.columns:
        ...
        # Flush out data that do not have min required yrs
        if min_yrs == -1:
            if yrs < 5:
                df[col] = np.nan
                hr_df.iloc[0] = np.nan
            if yrs < 10:
                hr_df.iloc[1:4] = np.nan
            if yrs < 29:
                hr_df.iloc[4:] = np.nan            
        else:
            if yrs < min_yrs:
...
def calc_stats_wy(ra_df, wy_df, periods, min_yrs):
    ...
    for styr, edyr in periods:
        ...
        ddf = process_stats_df(psdf, pdf, 'Water Year', '{}-{}'.format(styr, edyr), min_yrs)
        df = df.append(ddf)
...
def calc_stats_sn(ra_df, sn_df, periods, min_yrs):
    ...
    for styr, edyr in periods:
        for sn in sns:
            ...
            ddf = process_stats_df(psdf, pdf, sn, '{}-{}'.format(styr, edyr), min_yrs)
            df = df.append(ddf)
...
def calc_stats_mo(ra_df, mo_df, periods, min_yrs):
    ...
    for (styr,edyr), mo in [(x, y) for x in periods for y in range(1,13)]:
        ...
        ddf = process_stats_df(psdf, pdf, mo, '{}-{}'.format(styr, edyr), min_yrs)
        df = df.append(ddf)
...
def calc_stats(ra_df, wy_df, sn_df, mo_df, min_yrs, isHis=True):
    df = pd.DataFrame()
    
    if isHis:
        styr = ra_df.index.year[0]
        edyr = ra_df.index.year[-1]
        periods = [(styr, edyr)]
    else:
        #periods = [(1970, 1999), (1980, 2009), (2020, 2049), (2030, 2059),
        #       (2040, 2069), (2050, 2099), (2060, 2099), (2070, 2099)]
        periods = [(1970, 1999), (2020, 2049), (2040, 2069), (2070, 2099)]
        
    # Process water years
    df = df.append(calc_stats_wy(ra_df, wy_df, periods, min_yrs))    
    # Process seasonal
    df = df.append(calc_stats_sn(ra_df, sn_df, periods, min_yrs))
    # Process monthly
    df = df.append(calc_stats_mo(ra_df, mo_df, periods, min_yrs))
...
def csv_comment(df, out, comment, index=False):
    f = open(out, 'w')
    f.write(comment)
    df.to_csv(f, index=index, float_format='%.2f')
    f.close()
